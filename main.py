...
async def get_forward_message_user_id(msg_id: int):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT user_id FROM forward_messages WHERE msg_id = $1", msg_id
        )
...
async def is_user_banned(user_id: int):
    pool = get_pool()
    async with pool.acquire() as conn:
        result = await conn.fetchrow("SELECT * FROM banned WHERE user_id = $1", user_id)
        return bool(result)
...
async def get_forward_relation(button_msg_id: int):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT message_data, user_id FROM forward_relations WHERE button_msg_id = $1",
            button_msg_id,
        )
        if row:
...
async def statscmd(message: Message) -> None:
    if message.chat.id == GROUP_ID:
        ...
        async with pool.acquire() as conn:
            ...
            total_msgs = (await conn.fetchrow("SELECT COUNT(*) FROM forward_messages"))[
                0
            ]
    ...
    if message.chat.id == GROUP_ID:
        ...
        if message.media_group_id:
            ...
            media_group_timers[message.media_group_id] = asyncio.create_task(
                process_media_group_after_delay(message.media_group_id, suffix)
            )
            return
        ...
        if message_data:
            await save_forward_relation(
                sent_msg.message_id, message_data, message.from_user.id
            )
    ...
    try:
        ...
        if messages:
            ...
            if media:
                ...
                builder.add(InlineKeyboardButton(text="✅", callback_data="accept"))
                builder.add(InlineKeyboardButton(text="❌", callback_data="deny"))
                button_msg = await sent[0].reply(
                    "———————————————————", reply_markup=builder.as_markup()
                )
                await save_forward_relation(
                    button_msg.message_id, message_data, messages[0].from_user.id
                )
                for sent_msg in sent:
                    await save_forwarded_message(
                        sent_msg.message_id, messages[0].from_user.id
                    )
        ...
        if message_data["type"] == "text":
            ...
            for item in message_data["media"]:
                if item["type"] == "photo":
                    ...
                    media.append(
                        InputMediaVideo(media=item["file_id"], caption=item["caption"])
                    )
                elif item["type"] == "audio":
                    media.append(
                        InputMediaAudio(media=item["file_id"], caption=item["caption"])
                    )
